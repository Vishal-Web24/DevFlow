...
export default function KanbanBoard({ projectId }: Props) {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    dispatch(moveTaskOptimistic({ taskId: draggableId, status: newStatus, position: newPosition }));

    try {
      await dispatch(updateTask({ taskId: draggableId, projectId, status: newStatus, position: newPosition }));
    } catch {
            ...
            <div key={col.id} className={`flex-shrink-0 w-72 flex flex-col rounded-xl border-t-4 ${col.color} ${col.bg} border border-gray-200 dark:border-gray-800`}>
              ...
              <div className="p-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs font-medium px-1.5 py-0.5 rounded-full">{colTasks.length}</span>
                </div>
                {col.id === 'TODO' && (
                  <button onClick={() => dispatch(setCreateTaskModal(true))}
                    className="text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 text-xl leading-none transition-colors" title="Add task">+</button>
                )}
              </div>
        ...
        <div
          ...
          <div className="flex items-center justify-between mt-2.5 gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {task.assignee && (
                <img src={task.assignee.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(task.assignee.name)}&size=24&background=3B82F6&color=fff`}
